...
export class UsersService {
  ...
  async findAll(currentUser: User, targetStatus: UserStatus): Promise<User[]> {
    if (
      (!currentUser.status || currentUser.status === UserStatus.APPLICANT) &&
      (targetStatus === UserStatus.ALUMNI ||
    ...
    if (
      (currentUser.status === UserStatus.MEMBER ||
        currentUser.status === UserStatus.ALUMNI) &&
      targetStatus === UserStatus.APPLICANT
    ) {
      throw new UnauthorizedException();
    }
    ...
    if (targetStatus !== UserStatus.MEMBER) {
      ...
      users = await this.usersRepository.find({
        where: {
          $or: [
            { status: { $eq: UserStatus.MEMBER } },
            { status: { $eq: UserStatus.ADMIN } },
            { status: { $eq: UserStatus.RECRUITER } },
          ],
        },
      });
    }
